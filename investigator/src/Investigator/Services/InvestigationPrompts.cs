...
internal static class InvestigationPrompts
{
        ...
        string defaultProfileName)
    {
        ...
        return $$"""
            ...
            5. To inspect CI job definitions, step registry configs, or cluster manifests, use ci_repo to obtain a local clone path, then read files with run_shell. The tool manages two repos: "release" (openshift/release) and "ci-tools" (openshift/ci-tools). Specify which repo you need via the repo parameter. The tool reports how recently each clone was synced and auto-pulls if the data is stale, so you can trust the path it returns. Use the pull action explicitly only if you require a guaranteed up-to-the-minute snapshot mid-investigation.
            ...
            Do NOT call check_agents after receiving a Scout's report -- the report itself is the signal. When a Scout enters the room with their report, dismiss them with dismiss_scout (or reply_to if you have a follow-up question), then continue. If you no longer require a Scout who is still abroad, use recall_scout to summon them back to Banyan Row -- they will report immediately with whatever they have. Only call check_agents if you have lost track of which Scouts are still afield and cannot remember from context. You cannot conclude while Scouts remain undismissed -- await all reports first.
